...
test("Leap year 'true' when divisable by 4 and not 100", () => {
  const result = isLeapYear(2004);
  expect(result).toBe(true);
});

test("Leap year 'true' when divisable by 400", () => {
  const result = isLeapYear(2000);
  expect(result).toBe(true);
});

test("Leap year 'false' when neither divisable by 4 nor 400", () => {
  const result = isLeapYear(2001);
  expect(result).toBe(false);
});

test("datePlusDays is correct when adding 0 days", () => {
  const result = datePlusDays(new Date("2020-01-01"), 0);
  expect(result).toEqual(new Date("2020-01-01"));
});

test("datePlusDays is correct when adding 1 day in month", () => {
...
test("datePlusDays is correct when adding 1 day at month end", () => {
  const result = datePlusDays(new Date("2020-01-31"), 1);
  expect(result).toEqual(new Date("2020-02-01"));
});

test("datePlusDays is correct when adding -1 day at month start", () => {
  const result = datePlusDays(new Date("2020-02-01"), -1);
  expect(result).toEqual(new Date("2020-01-31"));
});

test("datePlusDays is correct when adding 365 days at year start", () => {
